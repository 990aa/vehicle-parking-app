...
class ParkingLot(db.Model):  # type: ignore
    ...
    price_per_hr = db.Column(
        db.Float, nullable=False
    )  # should this be decimal? float is easier, but maybe rounding issues
    # the address of the parking lot
    address = db.Column(db.String(100), nullable=False)
    # the pin code for the address
    pin_code = db.Column(
        db.String(10), nullable=False
    )  # i think 10 is enough, but what if someone has a weird pin
    ...
    )  # i had a bug where this was 0, so now always check in forms
    ...
    spots = db.relationship(
        "ParkingSpot", backref="parking_lot", cascade="all, delete-orphan"
    )
